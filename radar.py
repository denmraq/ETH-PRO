...
def analyze():
    ...
    # V1.17 directional hysteresis: a single new red/green candle must not flip
    # the 1h call. A reversal is accepted only when probability is strong or
    # multi-window evidence confirms it. This is deliberately stateful across
    # refreshes, but expires quickly so genuine reversals are not frozen out.
    try:
        prev_state=kv_get("forecast_1h_state_v2", {}) or {}
    except Exception:
        prev_state={}
    now_ts=time.time()
    new_dir=str(fwd['direction_1h']); new_prob=float(fwd['probability_1h'])
    prev_dir=str(prev_state.get('direction') or '')
    prev_ts=float(prev_state.get('ts') or 0.0)
    fresh_prev=prev_dir in ('LONG','SHORT') and (now_ts-prev_ts)<45*60
    if fresh_prev and prev_dir != new_dir:
        desired_sign=1 if new_dir=='LONG' else -1
        # Closed 5m + 15m structure must agree, OR mature flow must agree, OR
        # model probability must be decisively strong.
        z5=d5.iloc[-1]; z15x=d15.iloc[-1]
        struct5=1 if (z5['close']>z5['ema20'] and z5['ema20']>d5.iloc[-4]['ema20']) else (-1 if (z5['close']<z5['ema20'] and z5['ema20']<d5.iloc[-4]['ema20']) else 0)
        struct15=1 if (z15x['close']>z15x['ema20'] and z15x['ema20']>d15.iloc[-4]['ema20']) else (-1 if (z15x['close']<z15x['ema20'] and z15x['ema20']<d15.iloc[-4]['ema20']) else 0)
        flow_confirm=False
        if f5['coverage_min']>=4.0 and d5flow is not None:
            flow_confirm = (desired_sign>0 and d5flow>=8.0) or (desired_sign<0 and d5flow<=-8.0)
        structural_confirm=(struct5==desired_sign and struct15==desired_sign)
        if new_prob < 60.0 and not structural_confirm and not flow_confirm:
            # Keep previous direction but show low confidence; no WAIT state.
            fwd['direction_1h']=prev_dir
            fwd['probability_1h']=round(max(50.1, min(56.0, 100.0-new_prob)),1)
            warnings.append("1ч: одиночное краткое движение не подтвердило смену направления; сохранён предыдущий сигнал до подтверждения 5м/15м или потоком.")
    try:
        kv_set("forecast_1h_state_v2", {'direction':fwd['direction_1h'],'probability':fwd['probability_1h'],'ts':now_ts})
    except Exception:
        pass

    # V1.14: the main decision is the 1h forward direction from CURRENT price,
    ...
    trade_reason=f"Сейчас: {trade_signal} · вероятность {fwd['probability_1h']}% по forward-модели с учётом рыночной реакции на новости."
