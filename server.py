...
from push_service import public_key, configured as push_configured, send_all, send_one
...
app=FastAPI(title='ETH Entry Radar PRO V1.18')
...
@app.get('/api/health')
def health(): return {'ok':True,'version':'pro-1.18','push':push_configured(),'monitor':os.getenv('ENABLE_MONITOR','1')=='1'}
@app.get('/api/radar')
...
def push_subscribe(sub:PushSub):
    ...
    # Immediate self-test proves the iPhone subscription actually works instead of
    # showing a misleading green button and waiting for a future signal change.
    test=send_one(payload,'ETH Radar PRO','Push подключён. Уведомления о смене направления активны.')
    if not test.get('sent'):
        raise HTTPException(502,f"Подписка сохранена, но тестовый Push не отправлен: {test.get('error') or test.get('status')}")
    return {'ok':True,'test_sent':True}
