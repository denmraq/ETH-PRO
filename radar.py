...
def forward_outlook(price: float, direction: str, stop: float, target: float,
    ...
    """Forward-looking probabilistic model from the *current* market state.

    This is deliberately not a nearest-historical-analogue lookup.  It combines the
    live/most-recent closed multi-timeframe structure, momentum, flow, OI and
    volatility into a directional drift and then simulates possible 1-minute paths
    for the next 4 hours.  The output is a probability distribution, not knowledge
    of future prices.
    """
    ...
    # Signed features. Positive = upward pressure, negative = downward pressure.
    f1=np.tanh(0.9*gap_norm(d1m)+0.7*slope_norm(d1m)+0.45*ret_norm(d1m,4))
    ...
    # OI is interpreted jointly with recent 5m price direction: rising OI confirms
    # whichever side is currently moving; falling OI is treated as weaker evidence.
    ...
    if oi_change > 0.15:
        ...
        foi=0.0
    # Funding is only a small contrarian term.
    ffund=-float(np.clip(funding_pct/0.05,-1,1))*0.15

    # Different horizons use different weights. Fast layers matter more at 15m;
    # structural layers matter more at 60m.
    edge15=(0.24*f1+0.22*f5+0.14*f15+0.08*f1h+0.03*f4h+0.16*ff5+0.09*ff15+0.04*foi+ffund)
    edge60=(0.07*f1+0.14*f5+0.20*f15+0.19*f1h+0.10*f4h+0.10*ff5+0.12*ff15+0.07*foi+ffund)
    edge15=float(np.clip(edge15,-1.5,1.5)); edge60=float(np.clip(edge60,-1.5,1.5))

    atr15=max(float(d15.iloc[-1]['atr14']),1e-9)
    atr1h=max(float(d1.iloc[-1]['atr14']),1e-9)
    # Range probability rises when edge is weak and in RANGE regime.
    base_range15=0.48 if regime=='RANGE' else (0.34 if regime=='TRANSITION' else 0.24)
    base_range60=0.40 if regime=='RANGE' else (0.29 if regime=='TRANSITION' else 0.20)
    r15log=np.log(max(0.08,base_range15)) + 0.55*(1-abs(edge15))
    r60log=np.log(max(0.08,base_range60)) + 0.45*(1-abs(edge60))
    up15,down15,range15=_softmax3(1.75*edge15,-1.75*edge15,r15log)
    up60,down60,range60=_softmax3(1.55*edge60,-1.55*edge60,r60log)

    # Recent realized 1m volatility, with ATR-derived floor to prevent artificial
    # certainty during very quiet samples.
    rets=pd.to_numeric(d1m['close'],errors='coerce').pct_change().dropna().tail(120)
    ...
    # Convert edge to a modest one-minute drift. Drift is intentionally capped so
    # volatility still dominates most individual paths.
    drift1=float(np.clip(edge60*sigma1*0.16,-sigma1*0.22,sigma1*0.22))

    # Deterministic within the same closed 1m candle so refreshes do not flicker.
    ts=int(pd.Timestamp(d1m.iloc[-1]['close_time']).timestamp()//60)
    seed=(ts*1315423911 + int(round(price*100)) + int(round(edge60*10000))) & 0xffffffff
    rng=np.random.default_rng(seed)
    steps=240
    shocks=rng.normal(loc=drift1,scale=sigma1,size=(int(simulations),steps))
    # Mild momentum decay across horizon; current edge matters most earlier.
    decay=np.linspace(1.0,0.35,steps)
    shocks += drift1*(decay-1.0)
    paths=price*np.exp(np.cumsum(shocks,axis=1))

    if direction=='LONG':
        hit_tp=paths>=float(target); hit_sl=paths<=float(stop)
    else:
        hit_tp=paths<=float(target); hit_sl=paths>=float(stop)
    has_tp=hit_tp.any(axis=1); has_sl=hit_sl.any(axis=1)
    first_tp=np.where(has_tp,hit_tp.argmax(axis=1)+1,steps+1)
    first_sl=np.where(has_sl,hit_sl.argmax(axis=1)+1,steps+1)
    tp_first=first_tp<first_sl
    sl_first=first_sl<=first_tp
    neither=(~has_tp)&(~has_sl)
    # For neither, don't count as SL in ties from sentinel values.
    sl_first=sl_first & (~neither)
    n=float(simulations)
    tp_times=first_tp[tp_first]
    def qmin(vals,p):
        return int(round(float(np.quantile(vals,p))/5)*5) if len(vals) else None

    # Expected 60m distribution and structural breakout levels.
    p60=paths[:,59]
    q10,q50,q90=[float(x) for x in np.quantile(p60,[.10,.50,.90])]
    up_level=max(float(d15['high'].tail(20).max()),price+0.45*atr15)
    down_level=min(float(d15['low'].tail(20).min()),price-0.45*atr15)
    p_break_up=float((paths[:,:60].max(axis=1)>=up_level).mean())
    p_break_down=float((paths[:,:60].min(axis=1)<=down_level).mean())

    # Probability of reaching the actual trade targets inside the next 60m.
    # These are more actionable than a distant structural breakout threshold.
    def p_hit_60(level):
        if direction=='LONG':
            return float((paths[:,:60].max(axis=1)>=float(level)).mean())
        return float((paths[:,:60].min(axis=1)<=float(level)).mean())

    # Confidence = data agreement + directional separation, not success probability.
    signs=np.array([f1,f5,f15,f1h,f4h,ff5,ff15],dtype=float)
    agreement=abs(float(signs.mean()))
    separation=abs(up60-down60)
    confidence=int(round(np.clip(45+30*agreement+25*separation,35,95)))
    momentum_delta=float((edge15-edge60)*100)

    return {
        'up_15m':round(up15*100,1),'range_15m':round(range15*100,1),'down_15m':round(down15*100,1),
        'up_60m':round(up60*100,1),'range_60m':round(range60*100,1),'down_60m':round(down60*100,1),
        'expected_60m_low':round(q10,2),'expected_60m_mid':round(q50,2),'expected_60m_high':round(q90,2),
        'breakout_up_level':round(up_level,2),'breakdown_level':round(down_level,2),
        'p_breakout_up_60m':round(p_break_up*100,1),'p_breakdown_60m':round(p_break_down*100,1),
        'p_target_60m':round(p_hit_60(target)*100,1),
        'momentum_delta':round(momentum_delta,1),'forecast_confidence':confidence,
        ...
        'first_event_median_min':qmin(np.minimum(first_tp,first_sl)[np.minimum(first_tp,first_sl)<=steps],.50),
        'edge_15m':round(edge15,4),'edge_60m':round(edge60,4),
    }
...
def analyze():
    ...
    fwd=forward_outlook(price,direction,stop,tp2,d1m,d5,d15,d1,d4,d5flow,d15flow,oi_ch,funding,regime,4000)

    # V1.8: Forward Engine arbitrates whether the directional idea is tradable NOW.
    # Direction may remain LONG/SHORT, but a bad TP-vs-SL geometry becomes WAIT.
    ptp=float(fwd.get('p_tp_first') or 0.0); psl=float(fwd.get('p_sl_first') or 0.0)
    directional_prob=float(fwd.get('up_60m') if direction=='LONG' else fwd.get('down_60m'))
    blocked=(block_long if direction=='LONG' else block_short)
    if blocked:
        trade_signal='WAIT'; trade_reason='Вход заблокирован: цена/RSI растянуты для этого направления.'
    elif ptp + 8 < psl:
        trade_signal='WAIT'; trade_reason=f'ЖДАТЬ: TP раньше стопа {ptp:.1f}%, стоп раньше TP {psl:.1f}%.'
    elif directional_prob < 42:
        trade_signal='WAIT'; trade_reason=f'ЖДАТЬ: вероятность направления на 60 мин только {directional_prob:.1f}%.'
    elif fwd.get('forecast_confidence',0) < 50:
        trade_signal='WAIT'; trade_reason=f'ЖДАТЬ: уверенность прогноза {fwd.get("forecast_confidence",0)}/100.'
    else:
        trade_signal=direction; trade_reason=f'Вход подтверждён Forward Engine: направление {directional_prob:.1f}%, TP/SL {ptp:.1f}/{psl:.1f}%.'
    analog=time_to_event_analogs(d15_raw,direction,stop_atr,target_atr,16) if direction else {"analog_count":0}
    ...
    confidence += fm_pts
    quality.append(f"Forward path model: {fwd.get('path_count',0)} simulations (+{fm_pts}/15)")
    quality.append(f"Historical calibration sample: {ac} (secondary only)")
