...
def forward_outlook(price: float, direction: str, stop: float, target: float,
    ...
    edge1h=(0.06*f1+0.14*f5+0.20*f15+0.20*f1h+0.11*f4h+0.09*ff5+0.12*ff15+0.07*foi+ffund)
    edge6h=(0.02*f1+0.06*f5+0.15*f15+0.28*f1h+0.28*f4h+0.04*ff5+0.08*ff15+0.08*foi+0.60*ffund)
...
def current_price_levels(price: float, side: str, fwd: dict, atr15: float):
    """Build trade levels from the LIVE price, never from a future pullback entry.

    V1.11 philosophy: the user presses refresh and asks "if I enter now, which
    direction is more likely?".  Entry is therefore exactly the current price.
    TP ladder is horizon-aware (1h/6h/12h forward distribution), while the stop
    is an adverse 1h quantile with an ATR floor so it does not collapse onto price.
    """
    price=float(price); atr15=max(float(atr15),1e-9)
    step=max(0.55*atr15, abs(price)*0.0015)
    stop_floor=max(0.90*atr15, abs(price)*0.0030)
    if side=='SHORT':
        tp1=min(float(fwd['expected_1h_low']), price-step)
        tp2=min(float(fwd['expected_6h_low']), tp1-step)
        tp3=min(float(fwd['expected_12h_low']), tp2-step)
        stop=max(float(fwd['expected_1h_high']), price+stop_floor)
    else:
        tp1=max(float(fwd['expected_1h_high']), price+step)
        tp2=max(float(fwd['expected_6h_high']), tp1+step)
        tp3=max(float(fwd['expected_12h_high']), tp2+step)
        stop=min(float(fwd['expected_1h_low']), price-stop_floor)
...
def analyze():
    ...
    # V1.11: the main decision is the forward 1h direction from the CURRENT price.
    # Scores remain diagnostics, but they no longer define the live trade direction.
    trade_signal=fwd['direction_1h']
    trade_reason=f"Сейчас: {trade_signal} · вероятность {fwd['probability_1h']}% по forward-модели."
    ...
    confidence += fm_pts
    quality.append(f"Forward forecast model: OK (+{fm_pts}/15)")
    quality.append(f"Historical calibration: secondary only")
