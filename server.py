...
from radar import analyze, warm_future_history
...
app=FastAPI(title='ETH Entry Radar PRO V1.25')
...
@app.get('/')
def index():
    # GitHub mobile uploads can accidentally flatten/miss the static folder.
    # Serve either canonical static/index.html or a root-level duplicate.
    p = _first_existing(STATIC/'index.html', BASE/'index.html')
    if p:
        return FileResponse(p, media_type='text/html; charset=utf-8', headers={'Cache-Control':'no-store'})
    return HTMLResponse(
        '<h1>ETH Entry Radar PRO</h1><p>UI file missing: upload static/index.html or index.html.</p>',
        status_code=503,
...
def manifest():
    p = _first_existing(STATIC/'manifest.webmanifest', BASE/'manifest.webmanifest')
    if not p:
        raise HTTPException(404, 'manifest.webmanifest not found')
    return FileResponse(p, media_type='application/manifest+json')
@app.get('/api/health')
def health(): return {'ok':True,'version':'pro-1.25','push':push_configured(),'monitor':os.getenv('ENABLE_MONITOR','1')=='1'}
...
async def startup():
    init_db()
    # Build the FUTURE training history before the first /api/radar request.
    try:
        await asyncio.to_thread(warm_future_history)
    except Exception as e:
        try: kv_set('future_history_warmup_error', str(e))
        except Exception: pass
