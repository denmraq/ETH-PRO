...
def forward_outlook(price: float, direction: str, stop: float, target: float,
    ...
    edge1h=(0.15*f1+0.22*f5+0.23*f15+0.12*f1h+0.04*f4h+0.11*ff5+0.07*ff15+0.05*foi+0.20*ffund)
...
def current_price_levels(price: float, side: str, fwd: dict, atr15: float,
    ...
    """One scalp TP and one structural invalidation from the live price.

    TP is the nearest useful favorable level from 1m/5m/15m structure, bounded by
    the 1h forward distribution. STOP sits beyond the nearest adverse structural
    invalidation, with a small ATR buffer. 6h/12h never stretch either level.
    """
    ...
    highs=[]; lows=[]
    for df,n in ((d1m,45),(d5,36),(d15,24)):
        if df is None or not len(df):
            continue
        x=df.tail(n)
    ...
    if side=='LONG':
        ...
        candidates=sorted(v for v in highs if v>price+0.10*atr5)
        nearest=(candidates[0]-price) if candidates else None
        # nearby resistance first; never allow a tiny/noisy target.
        base=max(0.45*atr15, price*0.0015)
        fwd_cap=max(base, min(max(favorable*0.55,base), 1.25*atr15))
        move=min(nearest if nearest is not None else fwd_cap, fwd_cap)
        move=float(np.clip(move,base,max(base,1.35*atr15)))

        adverse=[v for v in lows if v<price-0.10*atr5]
        structural=max(adverse) if adverse else price-0.85*atr15
        stop=min(price-0.65*atr15, structural-0.12*atr5)
        stop=max(stop,price-1.55*atr15)
        return price, stop, price+move

    favorable=max(price-float(fwd['expected_1h_low']),0.0)
    candidates=sorted((v for v in lows if v<price-0.10*atr5), reverse=True)
    nearest=(price-candidates[0]) if candidates else None
    base=max(0.45*atr15, price*0.0015)
    fwd_cap=max(base, min(max(favorable*0.55,base), 1.25*atr15))
    move=min(nearest if nearest is not None else fwd_cap, fwd_cap)
    move=float(np.clip(move,base,max(base,1.35*atr15)))
    ...
    structural=min(adverse) if adverse else price+0.85*atr15
    stop=max(price+0.65*atr15, structural+0.12*atr5)
    stop=min(stop,price+1.55*atr15)
    return price, stop, price-move
