...
def _ticker(symbol):
    coin='ETH' if symbol.startswith('ETH') else 'BTC'
    attempts=[
        ('https://api.bybit.com/v5/market/tickers',{'category':'linear','symbol':symbol},lambda j: float(j['result']['list'][0]['lastPrice'])),
        ('https://www.okx.com/api/v5/market/ticker',{'instId':f'{coin}-USDT-SWAP'},lambda j: float(j['data'][0]['last'])),
        ('https://fapi.binance.com/fapi/v1/ticker/price',{'symbol':symbol},lambda j: float(j['price'])),
        (f'https://api.exchange.coinbase.com/products/{coin}-USD/ticker',{},lambda j: float(j['price'])),
    ]
    last=None
    for url,params,parse in attempts:
        try:
            r=requests.get(url,params=params,headers=UA,timeout=TIMEOUT); r.raise_for_status(); return parse(r.json())
        except Exception as e: last=e
    raise RuntimeError(f'All ticker sources failed for {symbol}: {last}')
