...
def forward_outlook(price: float, direction: str, stop: float, target: float,
    ...
    def horizon(idx):
        ...
        # Always emit LONG or SHORT. Probability is directional probability only.
        if up >= down:
            side='LONG'; prob=up*100
        else:
            side='SHORT'; prob=down*100
    ...
    h1=horizon(60); h6=horizon(360); h12=horizon(720)
...
def current_price_levels(price: float, side: str, fwd: dict, atr15: float):
    """Build ONE realistic scalp take-profit from the live price.

    PRO V1.14 deliberately exposes only TP1.  The 6h/12h horizons describe
    direction and must never stretch the active scalp target.  TP1 is tied to
    the current 15m volatility and the favorable 1h forward excursion.
    """
    ...
    # Scalp target: close enough to be useful, but still volatility-aware.
    # It is capped by 1h structure only; distant 6h/12h tails are ignored.
    floor=max(0.55*atr15, price*0.0018)
    ceiling=max(1.80*atr15, price*0.0100)
    raw=max(0.60*favorable, 0.75*atr15)
    move=float(np.clip(raw, floor, ceiling))
    risk=float(np.clip(max(adverse,0.95*atr15), 0.85*atr15, 1.85*atr15))

    tp1=price + sign*move
    stop=price - sign*risk
    return price, stop, tp1
...
def apply_news_to_forward(fwd: dict, news: dict | None):
    ...
    max_shift={'1h':10.0,'6h':7.0,'12h':5.0}
    out=dict(fwd)
    deltas={}
    for h in ('1h','6h','12h'):
        ...
        out[pkey]=round(max(long_p,100.0-long_p),1)
...
def analyze():
    ...
    live_entry, stop, tp1 = current_price_levels(price, trade_signal, fwd, atr15)
    tp2=tp3=None
