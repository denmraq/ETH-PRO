...
def apply_news_to_forward(fwd: dict, news: dict | None):
    """Blend current macro/news reaction into the forward direction.

    This is intentionally a bounded adjustment, not a headline override.
    The market model remains primary; confirmed high-impact news can move the
    probability more, while LOW/WAITING events barely affect it.
    """
    if not news or not news.get('focus'):
        return fwd, {'used': False, 'delta_1h': 0.0, 'reason': 'Нет активной новости'}
    focus=news['focus']
    score=float(focus.get('score') or 0.0)  # -100..100, already includes observed ETH/BTC reaction
    impact=str(focus.get('impact') or 'LOW').upper()
    confirm=str(focus.get('market_confirms') or 'WAITING').upper()
    impact_w={'LOW':0.25,'MEDIUM':0.60,'HIGH':1.00}.get(impact,0.25)
    confirm_w={'WAITING':0.25,'NO':0.15,'YES':1.00}.get(confirm,0.25)
    strength=(score/100.0)*impact_w*confirm_w

    # Maximum probability shifts in percentage points.  News matters most to
    # the immediate scalp horizon and decays with time.
    max_shift={'1h':7.0,'6h':3.0,'12h':2.0}
    out=dict(fwd)
    deltas={}
    for h in ('1h','6h','12h'):
        dkey=f'direction_{h}'; pkey=f'probability_{h}'
        d=str(out[dkey]); p=float(out[pkey])
        long_p=p if d=='LONG' else 100.0-p
        delta=max_shift[h]*strength
        long_p=float(np.clip(long_p+delta, 8.0, 92.0))
        out[dkey]='LONG' if long_p>=50 else 'SHORT'
        cap={'1h':82.0,'6h':74.0,'12h':70.0}[h]
        out[pkey]=round(min(max(long_p,100.0-long_p),cap),1)
        deltas[h]=round(delta,1)
    return out, {
        'used': abs(score)>=1,
        'delta_1h': deltas['1h'],
        'reason': f"Новости: {impact}, подтверждение {confirm}, оценка {score:+.0f}; вклад в 1ч {deltas['1h']:+.1f} п.п."
    }
...
def analyze():
    ...
    fwd=forward_outlook(price,direction,stop,tp2,d1m,d5,d15,d1,d4,flow5_for_model,flow15_for_model,float(oi_ch or 0.0),funding,regime,4000)
    try:
    ...
    # V1.18 PREDICTOR LOCK: forecasts are snapshots of the future, not a live
    # description of the current candle. Repeated manual refreshes inside the same
    # forecast window MUST return the same directional call.
    #
    # 1h forecast: recompute only after a new CLOSED 5m candle.
    # 6h forecast: recompute only after a new CLOSED 15m candle.
    # 12h forecast: recompute only once per 30m bucket.
    #
    # A normal reversal must survive two forecast windows. A very strong candidate
    # can flip immediately. This prevents LONG->SHORT->LONG noise while still
    # allowing a genuine regime change to appear promptly.
    def _forecast_lock(horizon, token, candidate_dir, candidate_prob, low, mid, high,
                       immediate_flip=68.0, normal_flip=60.0):
        key=f"predictor_lock_{horizon}_v18"
        try:
            st=kv_get(key, {}) or {}
        except Exception:
            st={}
        candidate_dir=str(candidate_dir).upper()
        candidate_prob=float(candidate_prob)
        # Same closed-candle token: the forecast is frozen.
        if st.get('token') == token and st.get('direction') in ('LONG','SHORT'):
            return st['direction'], float(st['probability']), float(st['low']), float(st['mid']), float(st['high'])

        prev_dir=str(st.get('direction') or '')
        prev_prob=float(st.get('probability') or 50.0)
        pending=str(st.get('pending_direction') or '')
        pending_count=int(st.get('pending_count') or 0)

        if prev_dir not in ('LONG','SHORT'):
            out_dir=candidate_dir; out_prob=candidate_prob; pending=''; pending_count=0
        elif candidate_dir == prev_dir:
            # Same view: smooth probability rather than jump with every new candle.
            out_dir=prev_dir
            out_prob=0.65*prev_prob + 0.35*candidate_prob
            pending=''; pending_count=0
        else:
            if candidate_prob >= immediate_flip:
                out_dir=candidate_dir; out_prob=candidate_prob; pending=''; pending_count=0
            else:
                if candidate_dir == pending:
                    pending_count += 1
                else:
                    pending=candidate_dir; pending_count=1
                if candidate_prob >= normal_flip and pending_count >= 2:
                    out_dir=candidate_dir; out_prob=candidate_prob; pending=''; pending_count=0
                else:
                    # Keep the old forecast. Confidence drifts gently toward neutral
                    # while an opposite candidate is forming. Keep the previous
                    # forward distribution too, so TP/SL geometry does not mix an
                    # old direction with a new opposite candidate distribution.
                    out_dir=prev_dir
                    out_prob=max(50.1, prev_prob - min(2.5, max(0.5,(candidate_prob-50.0)*0.12)))
                    low=float(st.get('low',low)); mid=float(st.get('mid',mid)); high=float(st.get('high',high))

        state={
            'token':token,'direction':out_dir,'probability':round(float(out_prob),1),
            'low':float(low),'mid':float(mid),'high':float(high),
            'candidate_direction':candidate_dir,'candidate_probability':round(candidate_prob,1),
            'pending_direction':pending,'pending_count':pending_count,'ts':time.time()
        }
        try:
            kv_set(key,state)
        except Exception:
            pass
        return state['direction'], state['probability'], state['low'], state['mid'], state['high']

    token1h=pd.Timestamp(d5.iloc[-1]['close_time']).isoformat()
    token6h=pd.Timestamp(d15.iloc[-1]['close_time']).isoformat()
    token12h=str(int(pd.Timestamp(d15.iloc[-1]['close_time']).timestamp() // (30*60)))

    x=_forecast_lock('1h',token1h,fwd['direction_1h'],fwd['probability_1h'],
                     fwd['expected_1h_low'],fwd['expected_1h_mid'],fwd['expected_1h_high'],68.0,60.0)
    fwd['direction_1h'],fwd['probability_1h'],fwd['expected_1h_low'],fwd['expected_1h_mid'],fwd['expected_1h_high']=x
    x=_forecast_lock('6h',token6h,fwd['direction_6h'],fwd['probability_6h'],
                     fwd['expected_6h_low'],fwd['expected_6h_mid'],fwd['expected_6h_high'],66.0,58.0)
    fwd['direction_6h'],fwd['probability_6h'],fwd['expected_6h_low'],fwd['expected_6h_mid'],fwd['expected_6h_high']=x
    x=_forecast_lock('12h',token12h,fwd['direction_12h'],fwd['probability_12h'],
                     fwd['expected_12h_low'],fwd['expected_12h_mid'],fwd['expected_12h_high'],65.0,57.0)
    fwd['direction_12h'],fwd['probability_12h'],fwd['expected_12h_low'],fwd['expected_12h_mid'],fwd['expected_12h_high']=x
    ...
    # V1.18: main decision is the LOCKED 1h future forecast, not the latest candle,
    ...
    trade_reason=f"Прогноз на ближайший час: {trade_signal} · {fwd['probability_1h']}%. Пересчёт только после новой закрытой 5м свечи."
