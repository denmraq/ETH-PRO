...
def adaptive_forward_plan(direction: str, price: float, entry_low: float, entry_high: float, stop: float,
    ...
    if direction=="LONG":
        ...
        # Candidate future resistance levels: unused old targets, historical swings, then ATR projections.
        cands=[x for x in t if x > price+eps]
        cands += [float(x) for x in d1["high"].tail(80).values if float(x) > price+eps]
        base=max(price, rhi, fast_close)
        cands += [base+0.55*atr15, base+1.10*atr15, base+1.80*atr15, base+2.60*atr15]
        future=sorted({round(x,8) for x in cands if x > price+eps})
    else:
        ...
        cands=[x for x in t if x < price-eps]
        cands += [float(x) for x in d1["low"].tail(80).values if float(x) < price-eps]
        base=min(price, rlo, fast_close)
        cands += [base-0.55*atr15, base-1.10*atr15, base-1.80*atr15, base-2.60*atr15]
        future=sorted({round(x,8) for x in cands if x < price-eps}, reverse=True)

    # Guarantee three actual forward levels even when history has no nearby swing.
    step=max(0.45*atr15, 1.2*atr5, 3.0*atr1)
    while len(future)<3:
        k=len(future)+1
        future.append(price+k*step if direction=="LONG" else price-k*step)
...
def analyze():
    ...
    if direction and all(x is not None for x in (entry_low,entry_high,stop,tp1,tp2,tp3)):
        ...
        if target_rollover:
            ...
            warnings.append(f"Dynamic target rollover active: {targets_hit} prior target(s) already consumed; forward targets rebuilt from 1m/5m structure.")
