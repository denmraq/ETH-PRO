...
def current_price_levels(price: float, side: str, fwd: dict, atr15: float,
    ...
    min_tp=15.0
    vol_tp=float(np.clip(0.95*atr15, min_tp, 35.0))
    if side=='LONG':
    ...
    target_move=max(min_tp, min(50.0, max(vol_tp, 0.55*forward)))
    ...
    min_risk=max(7.0, min(14.0, 0.40*atr15))
    # If volatility requires a wider valid stop, widen TP rather than publish bad R/R.
    target_move=max(target_move, 1.30*min_risk)
    max_risk=max(min_risk, min(22.0, target_move/1.30))  # reward/risk >= 1.30
