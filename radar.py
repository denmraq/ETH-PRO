...
_H5_CACHE = {"ts": 0.0, "df": None, "source": None}

def _normalize_5m_rows(rows) -> pd.DataFrame:
    if not rows:
        return pd.DataFrame()
...
def _deep_bybit_5m(target:int) -> pd.DataFrame:
    rows=[]; end=None
    while len(rows)<target:
        lim=min(1000,target-len(rows))
        params={"category":CATEGORY,"symbol":SYMBOL,"interval":"5","limit":lim}
        ...
        if end is not None and oldest>=end: break
        end=oldest-1
        if len(batch)<lim: break
        time.sleep(0.02)
    return _normalize_5m_rows([[r[0],r[1],r[2],r[3],r[4],r[5],r[6]] for r in rows])

def _deep_okx_5m(target:int) -> pd.DataFrame:
    # OKX history-candles permits paging farther back than the ordinary candles endpoint.
    rows=[]; after=None
    while len(rows)<target:
        lim=min(100,target-len(rows))
        params={"instId":OKX_INST,"bar":"5m","limit":lim}
        ...
        if len(batch)<lim: break
        time.sleep(0.02)
    norm=[[r[0],r[1],r[2],r[3],r[4],r[5],r[7] if len(r)>7 else float(r[4])*float(r[5])] for r in rows]
    return _normalize_5m_rows(norm)

def _deep_binance_5m(target:int) -> pd.DataFrame:
    rows=[]; end_ms=None
    while len(rows)<target:
        lim=min(1500,target-len(rows))
        params={"symbol":SYMBOL,"interval":"5m","limit":lim}
        if end_ms is not None: params["endTime"]=int(end_ms)
        batch=_binance_get("/fapi/v1/klines",params)
        if not batch: break
        rows.extend(batch)
        oldest=min(int(r[0]) for r in batch)
        if end_ms is not None and oldest>=end_ms: break
        end_ms=oldest-1
        if len(batch)<lim: break
        time.sleep(0.02)
    return _normalize_5m_rows([[r[0],r[1],r[2],r[3],r[4],r[5],r[7]] for r in rows])

def _deep_coinbase_5m(target:int) -> pd.DataFrame:
    # Coinbase returns max ~300 candles/request. Walk backward in 25-hour chunks.
    rows=[]; end=pd.Timestamp.now(tz='UTC').floor('5min')
    chunk=299*5
    while len(rows)<target:
        start=end-pd.Timedelta(minutes=chunk)
        params={"granularity":300,"start":start.isoformat(),"end":end.isoformat()}
        batch=_coinbase_get(f"/products/{COINBASE_PRODUCT}/candles",params)
        if not batch: break
        rows.extend(batch)
        oldest=min(int(r[0]) for r in batch)
        new_end=pd.to_datetime(oldest,unit='s',utc=True)-pd.Timedelta(seconds=1)
        if new_end>=end: break
        end=new_end
        if len(batch)<20: break
        time.sleep(0.03)
    norm=[[int(r[0])*1000,r[3],r[2],r[1],r[4],r[5],float(r[4])*float(r[5])] for r in rows]
    return _normalize_5m_rows(norm)

def _history_5m_deep(target: int = 6000) -> pd.DataFrame:
    """Load deep CLOSED 5m history for FUTURE-labelled training.

    Critical rule: never fall back to only ~300 candles for a 12h predictor.
    Render can block one exchange, so we try several deep pageable sources.  The
    current candle is not used as the label; every historical row is labelled by
    what happened AFTER that row.
    """
    now=time.time(); cached=_H5_CACHE.get("df")
    if isinstance(cached,pd.DataFrame) and len(cached)>=600 and now-float(_H5_CACHE.get("ts",0))<900:
        return cached.copy()
    attempts=[]; best=pd.DataFrame(); best_source=None
    for source,loader in (("Bybit perpetual",_deep_bybit_5m),("OKX perpetual",_deep_okx_5m),("Binance perpetual",_deep_binance_5m),("Coinbase spot",_deep_coinbase_5m)):
        try:
            df=loader(target)
            attempts.append(f"{source}:{len(df)}")
            if len(df)>len(best): best=df; best_source=source
            # 1200 bars gives >4 days of 5m states and leaves ample samples after 12h labels.
            if len(df)>=1200:
                best=df; best_source=source; break
        except Exception as e:
            attempts.append(f"{source}:{type(e).__name__}")
    if len(best)<600:
        try:
            df=klines("5m",1000,True)
            attempts.append(f"standard:{len(df)}")
            if len(df)>len(best): best=df; best_source="standard fallback"
        except Exception as e:
            attempts.append(f"standard:{type(e).__name__}")
    if len(best)<360:
        raise RuntimeError("Недостаточно глубокой 5м истории для предсказателя; " + ", ".join(attempts))
    _H5_CACHE.update({"ts":now,"df":best.copy(),"source":best_source})
    return best.copy()

def warm_future_history() -> dict:
    """Preload predictor history at server startup so the first user request is ready."""
    df=_history_5m_deep(6000)
    return {"bars":int(len(df)),"source":_H5_CACHE.get("source")}
...
def _ridge_logistic_probability(X: np.ndarray, y: np.ndarray, q: np.ndarray, sample_w: Optional[np.ndarray]=None) -> Optional[float]:
    """Small dependency-free regularised logistic model.

    This is a second opinion to the nearest-analogue model. It is trained only
    on historical states whose future outcome is already known.
    """
    if len(X)<120 or len(np.unique(y))<2: return None
    ...
    Z=np.clip((X-med)/scale,-6,6); qz=np.clip((q-med)/scale,-6,6)
    Z=np.column_stack([np.ones(len(Z)),Z]); qz=np.r_[1.0,qz]
    w=np.zeros(Z.shape[1]); sw=np.ones(len(Z)) if sample_w is None else np.asarray(sample_w,float)
    sw=sw/max(float(np.mean(sw)),1e-9)
    lr=0.08; l2=0.08
    for _ in range(180):
        z=np.clip(Z@w,-20,20); p=1.0/(1.0+np.exp(-z))
        grad=(Z.T@((p-y)*sw))/len(Z)
        grad[1:]+=l2*w[1:]
        w-=lr*np.clip(grad,-3,3)
    prob=1.0/(1.0+np.exp(-float(np.clip(qz@w,-20,20))))
...
def _analog_horizon(feat: pd.DataFrame, horizon_bars: int, price: float, k: int = 100):
    ...
    min_hist=max(110, min(180, horizon_bars+45))
    ...
    analog_long=0.72*first_long + 0.28*term_long + 0.50*unresolved*0.28

    # Independent global model: learns the sign of the FUTURE horizon return.
    # It reduces overreaction when the nearest-neighbour set is accidentally one-sided.
    allX=hist[cols].astype(float).to_numpy(); ally=(hist['future_ret'].to_numpy()>0).astype(float)
    age_w=np.linspace(0.55,1.0,len(hist))
    ml_long=_ridge_logistic_probability(allX,ally,q,age_w)
    if ml_long is None:
        long_p=analog_long
    else:
        # Analogues dominate because they preserve market regime; logistic layer is a stabilizer.
        long_p=0.76*analog_long+0.24*ml_long
    long_p=float(np.clip(long_p,0.03,0.97))
    short_p=1.0-long_p
...
def future_analog_outlook(price: float, flow5: Optional[float], flow15: Optional[float],
    ...
    if not all((h1,h6,h12)):
        raise RuntimeError(f'Недостаточно истории для модели будущего движения: доступно {len(raw)} закрытых 5м свечей')
    ...
    return {
      ...
      'future_history_bars':int(len(raw)), 'future_history_source':_H5_CACHE.get('source'),
    }
...
def analyze():
    ...
    # V1.20: a new forecast is clocked, not candle-clocked. A newly relevant
