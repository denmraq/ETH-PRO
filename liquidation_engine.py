...
def apply_to_forward(fwd: dict, liq: dict) -> dict:
    """Apply liquidation bias defensively. This function must never break /api/radar."""
    try:
...
def safe_snapshot(price: float) -> dict:
    """Non-fatal wrapper used by the main radar endpoint.

    External liquidation providers are optional. A bad key, rate limit, schema change,
    timeout, or provider outage must never turn the whole radar into HTTP 500.
    """
    try:
        data = snapshot(price)
        return data if isinstance(data, dict) else LiquidationSnapshot(note="Некорректный ответ поставщика карты ликвидаций").to_dict()
    except Exception as e:
        return LiquidationSnapshot(
            available=False,
            provider="Недоступно",
            source_mode="none",
            note=f"Карта ликвидаций временно отключена: {type(e).__name__}. Основной прогноз продолжает работать.",
        ).to_dict()
